...
public class SyncBenchmarkTest {
    ...
    public static void main(String[] args) {
        if (args.length != 1) {
            System.out.println("usage: SyncBenchmarkTest threadNum");
            System.exit(-1);
        }
        ...
        options.setChannelType(ChannelType.POOLED_CONNECTION);
//        options.setWorkThreadNum(2);
        ...
        for (int i = 0; i < threadNum; i++) {
            ...
            threads[i] = new Thread(new ThreadTask(messageBytes, sendInfos[i], echoService),
                    "work-thread-" + i);
            threads[i].start();
        }
        ...
        public void run() {
            ...
            while (!stop) {
                try {
                    ...
                    log.debug("sync call success, elapseTimeNs={}, response={}",
                            elapseTimeNs, response.getMessage());
                } catch (Exception ex) {
