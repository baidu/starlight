...
public class BenchmarkTest {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        SendInfo[] sendInfos =
                new SendInfo[threadNum];
        Thread[] threads = new Thread[threadNum];
        for (int i = 0; i < threadNum; i++) {
            sendInfos[i] = new SendInfo();
            threads[i] = new Thread(new ServerPushTask(rpcServer, messageBytes, sendInfos[i], proxyPushApi),
                    "work-thread-" + i);
            threads[i].start();
        }
    ...
    //   class EchoCallback implements RpcCallback<Echo.EchoResponse> {
        ...
        public void run() {
            ...
            while (!stop) {
                ...
                try {
                    ...
                    PushResult r = serverSideUserPushApi.clientReceive("c1", p);
