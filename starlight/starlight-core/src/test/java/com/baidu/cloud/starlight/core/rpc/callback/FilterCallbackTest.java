...
import com.baidu.cloud.starlight.core.filter.TestFilter;
...
public class FilterCallbackTest {
    ...
    public void onResponse() {
        ...
        FilterCallback filterCallback = new FilterCallback(bizWrapCallback, new TestFilter(), request);
        filterCallback.addTimeout(timeout);
        filterCallback.onResponse(response);
    }

    @Test
    public void onResponseCallbackNull() {
        FilterCallback filterCallback = new FilterCallback(null, new TestFilter(), request);
        filterCallback.onResponse(response);
    }
    ...
    @Test
    public void onErrorCallbackNull() {
        FilterCallback filterCallback = new FilterCallback(null, new TestFilter(), request);
        filterCallback
            .onError(new StarlightRpcException(StarlightRpcException.INTERNAL_SERVER_ERROR, "Internal server error"));
    }
    ...
    public void getRequest() {
        BizWrapCallback bizWrapCallback = new BizWrapCallback(callback, request);
        FilterCallback filterCallback = new FilterCallback(bizWrapCallback, new TestFilter(), request);
        Assert.assertEquals(filterCallback.getRequest(), request);
    }
}